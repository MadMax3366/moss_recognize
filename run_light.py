...
def find_and_crop_quadrat(image, filename=None, output_size=(500, 500)):
    """Detect quadrat region and perform perspective crop. If not found, fallback to center crop."""
    DEBUG = False
    ...
    if best_box is None:
        print("⚠️ Quadrat not found. Falling back to center crop.")
        side_ratio = 0.88
...
def detect_moss_by_rgb_gray(image):
    """Detect moss regions using RGB and grayscale rules."""
...
def visualize_results(image, moss_mask, image_path):
    """Overlay moss detection mask and save side-by-side results."""
    overlay_color = (0, 0, 255)  # Selection blue in BGR
    debug_dir = "debug_pic"
...
def calculate_coverage(moss_mask):
    """Calculate moss coverage rate based on mask."""
...
WRITE_EVERY = 10  # Save to CSV every N images
...
if __name__ == "__main__":
    ...
    # Recursively search for jpg/jpeg files
    ...
    for idx, image_path in enumerate(tqdm(image_files, desc="Processing", unit="image"), 1):
        ...
        if not os.path.exists(image_path):
            tqdm.write(f"⚠️ Skipping, file not found: {image_path}")
            continue
        ...
        try:
            ...
            tqdm.write(f"❌ Error processing {image_path}, Reason: {e}")
            result = {
                "Image Path": image_path,
                "Quadrat Detected": "No",
                "Moss Coverage (%)": 0.0
        ...
        if len(results_buffer) >= WRITE_EVERY or idx == len(image_files):
            ...
            tqdm.write(f"📥 Saved {len(results_buffer)} records to {CSV_PATH}")
